...
function sword.check_update
    ...
    # if not set -q sword_updated
        ...
        if string.equals "$version_local_git" "$version_remote_git"
            info "sword+fish is up-to-date... "(color aqua)"("(color beige)(sword.version)(color aqua)")"
            set -xg sword_updated
            return 1
        else
            info "sword+fish update available... "(color tomato)"("(color darkorange)$version_local_git(color tomato)")"(color normal)"~>"(color palegreen)"("(color aqua)$version_remote_git(color palegreen)")"
            set -xg sword_updated
            return 0
        end
    # else
